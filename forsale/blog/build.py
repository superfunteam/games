...
""" + FOOT)
    ...
    side = "".join(
      f'''          <a class="j-item" href="/forsale/blog/{r["slug"]}/">
            <span class="j-date">{pretty(r["date"])}</span>
            <span class="j-title">{html.escape(r["title"])}</span>
          </a>
''' for r in recent)
    block = f'''<!-- BLOG:LATEST -->
  <section class="journal">
    <div class="wrap">
      <div class="j-head">
        ...
        <a class="j-all" href="/forsale/blog/">All posts</a>
      </div>
      <div class="j-grid">
        <a class="j-feature" href="/forsale/blog/{latest["slug"]}/">
          <span class="j-thumb">{cover_svg(latest["slug"], 20, 15)}</span>
          <span class="j-date">{pretty(latest["date"])}</span>
